...
class Preprocessor$1 {
  ...
  advance() {
    ...
    const isCommonValidRange = cp > 0x1f && cp < 0x7f || cp === $$6.LINE_FEED || cp === $$6.CARRIAGE_RETURN || cp > 0x9f && cp < 0xfdd0;
  ...
  _ensureHibernation() {
    if (this.preprocessor.endOfChunkHit) {
      ...
      this.tokenQueue.push({
        type: Tokenizer$4.HIBERNATION_TOKEN
      });
      return true;
    }
    return false;
...
class FormattingElementList$1 {
  ...
  insertMarker() {
    this.entries.push({
      type: FormattingElementList$1.MARKER_ENTRY
    });
    this.length++;
  }
...
function hashObject(object, {
  ...
  return crypto__default["default"].createHash(algorithm).update(JSON.stringify(sortKeys(object, {
    deep: true
  ...
  return async tree => {
    ...
    visit(tree, 'element', (element, index, parent) => {
      ...
      element.tagName === 'pre' && Array.isArray(element.children) && element.children.length === 1 && isElement(element.children[0]) && element.children[0].tagName === 'code' && typeof element.children[0].properties === 'object' && Array.isArray(element.children[0].properties.className) && typeof element.children[0].properties.className[0] === 'string' && element.children[0].properties.className[0].startsWith('language-')) {
        ...
        const lang = element.children[0].properties.className[0].replace('language-', '');
